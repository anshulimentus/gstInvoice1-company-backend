...
export class CustomerService {
  ...
  async create(createCustomerDto: CreateCustomerDto): Promise<Customer> {
    ...
    if (
      !savedCustomer.wallet_address ||
      !savedCustomer.name ||
      !savedCustomer.gstNumber
    ) {
      throw new BadRequestException(
        'Missing required fields for blockchain transaction',
      );
    }
    ...
    try {
      if (tx.events && tx.events.CustomerAdded) {
        blockchainCustomerId = parseInt(
          tx.events.CustomerAdded.returnValues.customerId,
        );
        console.log(
          '🔢 Blockchain Customer ID (from event):',
          blockchainCustomerId,
        );
      } else {
        // Fallback: query totalCustomers if event is not available
        const totalCustomers = await this.contract.methods
          .totalCustomers()
          .call();
        blockchainCustomerId = parseInt(totalCustomers.toString());
        console.log(
          '🔢 Blockchain Customer ID (fallback from totalCustomers):',
          blockchainCustomerId,
        );
      }
    } catch (error) {
      ...
      throw new InternalServerErrorException(
        'Failed to get blockchain customer ID',
      );
    }
  ...
  async update(
    ...
    if (gstNumber && gstNumber !== customer.gstNumber) {
      ...
      if (existingCustomer && existingCustomer.id !== customer.id) {
        throw new BadRequestException(
          `Customer with GST number ${gstNumber} already exists.`,
        );
      }
    }
  ...
  async remove(id: string): Promise<void> {
    ...
    try {
      await this.contract.methods
        .deleteCustomer(customer.blockchainCustomerId)
        .send({ from: this.account });
    } catch (error) {
      throw new InternalServerErrorException('Blockchain transaction failed');
    }
