...
export class ProductService {
  ...
  constructor(
    ...
    console.log(
      '🚀 ~ ProductService ~ sanitizedPrivateKey:',
      sanitizedPrivateKey,
    );
    try {
      const account =
  ...
  async prepareCreate(
  ...
  ): Promise<{ transactionData: any; productData: any }> {
    ...
    const transactionData = await this.prepareTransaction(
      encodedABI,
  ...
  async completeCreate(
    ...
    try {
      if (receipt.events && receipt.events.ProductAdded) {
        blockchainProductId = parseInt(
          receipt.events.ProductAdded.returnValues.productID,
        );
        console.log(
          '🔢 Blockchain Product ID (from event):',
          blockchainProductId,
        );
      } else {
        // Fallback: query totalProducts if event is not available
        const totalProducts = await this.contract.methods
          .totalProducts()
          .call();
        blockchainProductId = parseInt(totalProducts.toString());
        console.log(
          '🔢 Blockchain Product ID (fallback from totalProducts):',
          blockchainProductId,
        );
      }
    } catch (error) {
      ...
      throw new InternalServerErrorException(
        'Failed to get blockchain product ID',
      );
    }
    ...
    console.log(
      '✅ Product saved to database with transaction hash:',
    ...
    try {
      ...
      try {
        if (tx.events && tx.events.ProductAdded) {
          blockchainProductId = parseInt(
            tx.events.ProductAdded.returnValues.productID,
          );
          console.log(
            '🔢 Blockchain Product ID (from event):',
            blockchainProductId,
          ...
          console.log(
            '📋 ProductAdded event:',
            tx.events.ProductAdded.returnValues,
          );
        } else {
          // Fallback: query totalProducts if event is not available
          const totalProducts = await this.contract.methods
  ...
  async update(
    ...
    // Validate GST rate according to contract requirements
    const gstRate = Math.round(
      updateProductDto.gstRate ?? existingProduct.gstRate,
    );
    if (gstRate <= 0 || gstRate > 100) {
      throw new BadRequestException('GST rate must be between 1 and 100');
    }
    ...
    try {
      ...
      console.error(
        '❌ Blockchain update failed - aborting database update:',
        error,
      );
      throw new BadRequestException(
        'Blockchain update failed: ' + error.message,
      );
    }

    // Step 2: ONLY AFTER blockchain success, update the database
    console.log(
      '💾 Blockchain transaction successful - now updating database...',
    );
    ...
    console.log(
      '📋 Old transaction hash in DB:',
      existingProduct.transactionHash,
    );
    ...
    console.log(
      '📋 Transaction hash after setting on entity:',
      existingProduct.transactionHash,
    );
    ...
    console.log(
      '✅ Database updated successfully after blockchain transaction',
    );
    console.log(
      '📋 Final transaction hash in saved product:',
      savedProduct.transactionHash,
    );
    return savedProduct;
  }
  ...
  async remove(id: number): Promise<void> {
    ...
    try {
      ...
      console.log(
        '📋 Attempting to delete product with blockchainProductId:',
        existingProduct.blockchainProductId,
      );
      console.log('📋 Database productID:', existingProduct.productID);

      const receipt = await this.contract.methods
        .deleteProduct(existingProduct.blockchainProductId)
        .send({ from: this.account });
      console.log(
        '✅ Product deleted from blockchain with blockchainProductId:',
        existingProduct.blockchainProductId,
        'transaction:',
        receipt.transactionHash,
      );
      ...
      // CRITICAL: If blockchain transaction fails, ABORT and don't touch the database
      console.error(
        '❌ Blockchain delete failed - aborting database deletion:',
        error,
      );
      console.error('❌ Error details:', {
        message: error.message,
      ...
      if (error.message && error.message.includes('Product')) {
        console.error(
          '❌ Product not found on blockchain - possible data inconsistency',
        );
        console.error(
          "❌ Database has product but blockchain doesn't - manual cleanup may be needed",
        );
        console.error(
          '💡 Suggestion: Check if this product was created before blockchainProductId was implemented',
        );
        console.error(
          '💡 Or verify that the blockchainProductId value is correct',
        );
      }

      throw new BadRequestException(
        'Blockchain delete failed: ' + error.message,
      );
    }

    // Step 2: ONLY AFTER blockchain success, delete from the database
    console.log(
      '💾 Blockchain transaction successful - now deleting from database...',
    );
    await this.productRepository.delete(id);
    console.log(
      `✅ Product with ID ${id} deleted from database after blockchain transaction`,
    );
  }

  // Method to check data consistency between database and blockchain
  async checkBlockchainConsistency(): Promise<{
    consistent: Product[];
    inconsistent: Product[];
  }> {
    console.log(
      '🔍 Checking data consistency between database and blockchain...',
    );
    ...
    for (const product of allProducts) {
      try {
        ...
        const blockchainProduct = await this.contract.methods
          .products(product.blockchainProductId)
          .call();

        if (blockchainProduct && blockchainProduct.productID) {
          console.log(
            `✅ Product ${product.productID} (blockchain ID: ${product.blockchainProductId}) exists on blockchain`,
          );
          consistent.push(product);
        } else {
          console.log(
            `❌ Product ${product.productID} (blockchain ID: ${product.blockchainProductId}) not found on blockchain`,
          );
          inconsistent.push(product);
        }
      } catch (error) {
        console.log(
          `❌ Error checking product ${product.productID} (blockchain ID: ${product.blockchainProductId}):`,
          error.message,
        );
        inconsistent.push(product);
      }
    ...
    console.log(
      `📊 Consistency check complete: ${consistent.length} consistent, ${inconsistent.length} inconsistent`,
    );
