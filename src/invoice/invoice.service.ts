...
export class InvoiceService {
  ...
  constructor(
    ...
    const sanitizedPrivateKey = this.privateKey.startsWith('0x')
      ...
      : '0x' + this.privateKey;
    console.log(
      '🚀 ~ InvoiceService ~ sanitizedPrivateKey:',
      sanitizedPrivateKey,
    );
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        doc
          .fontSize(24)
          .fillColor('white')
          .text('Eastern Supplies Co.', 80, 30);
        doc
          .fontSize(12)
          .fillColor('white')
          .text('Professional Services', 80, 55);
        ...
        doc
          .fontSize(10)
          .fillColor('white')
          .text(`INVOICE NO: ${invoice.invoiceNo}`, 450, 75);
        doc
          .fontSize(10)
          .fillColor('white')
          .text(`DATE: ${formattedDate}`, 450, 90);
        ...
        if (invoice.buyer) {
          ...
          yPos += 15;
          doc.fontSize(9).fillColor('#666666');
          doc.text(` ${invoice.buyer.billingAddress}`, 320, yPos, {
            width: 200,
          });
          yPos += 15;
        ...
        invoice.items.forEach((item, index) => {
          ...
          doc
            .fontSize(7)
            .fillColor('#888888')
            .text(`Serial: ${item.serialNo}`, 40, yPos + 18);
          doc.text(
        ...
        doc
          .fontSize(9)
          .fillColor('#666666')
          .text(invoice.paymentTerms || 'Due on Receipt', 40, yPos);
        ...
        if (invoice.transactionHash) {
          ...
          doc
            .fontSize(9)
            .fillColor(textColor)
            .text('Transaction Details:', 40, yPos);
          yPos += 12;
          doc
            .fontSize(8)
            .fillColor('#666666')
            .text(`Blockchain: ${invoice.transactionHash}`, 40, yPos, {
              width: 200,
            });
        }
        ...
        doc.text(
          `${Number(invoice.totalTaxableValue).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
          480,
          totalsY,
        );
        ...
        doc.text(
          `${Number(invoice.totalGstAmount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
          480,
          totalsY,
        );
  ...
  async createInvoice(createInvoiceDto: CreateInvoiceDto): Promise<Invoice> {
    try {
      ...
      if (
        ...
        throw new Error(
          'Invoice item arrays must be of the same non-zero length',
        );
      }
    ...
    try {
      ...
      if (!dbInvoices || dbInvoices.length === 0) {
        throw new NotFoundException(
          `No invoices found for tenantId: ${tenantId}`,
        );
      }

      // 2. ✅ Fetch all invoices from blockchain
      const chainInvoicesRaw = await this.contract.methods
        .getAllInvoices()
        .call();
      ...
      const mergedInvoices = await Promise.all(
        dbInvoices.map(async (dbInv) => {
          ...
          if (chainInv) {
            return {
              ...
              totalTaxableValue:
                chainInv.totalTaxableValue || dbInv.totalTaxableValue,
  ...
  async findInvoicesByBuyerWallet(walletAddress: string): Promise<Invoice[]> {
    ...
    if (!customer) {
      throw new NotFoundException(
        `Customer with wallet address ${walletAddress} not found`,
      );
    }
    ...
    if (invoices.length === 0) {
      throw new NotFoundException(
        `No invoices found for wallet address ${walletAddress}`,
      );
    }
   ...
   * @returns Promise<Invoice[]>
   */
  async findPendingInvoicesByBuyerWallet(
  ...
  private async createInvoiceOnChain(
    invoiceNo: string,
    grandTotal: number,
  ): Promise<string> {
    try {
      const grandTotalWei = this.web3.utils.toWei(
        grandTotal.toString(),
        'ether',
      );
      ...
      const gasEstimate = await this.contract.methods
        .createInvoice(invoiceNo, grandTotalWei)
        .estimateGas({
          from: this.account,
        });
      ...
      throw new InternalServerErrorException(
        'Failed to create invoice on blockchain',
      );
    }
  }

  private async updateInvoiceOnChain(
    invoiceNo: string,
    grandTotal: number,
  ): Promise<string> {
    try {
      ...
      const gasPrice = await this.web3.eth.getGasPrice();
      const gasEstimate = await this.contract.methods
        .updateInvoice(invoiceNo, grandTotalWei)
        .estimateGas({
          from: this.account,
        });
      ...
      throw new InternalServerErrorException(
        'Failed to update invoice on blockchain',
      );
    }
  }
  ...
  private async deleteInvoiceOnChain(invoiceNo: string): Promise<string> {
    try {
      ...
      throw new InternalServerErrorException(
        'Failed to delete invoice on blockchain',
      );
    }
  }
  ...
  async convertBigIntToString(obj: any): Promise<any> {
    if (Array.isArray(obj)) {
      const result = await Promise.all(
        obj.map((item) => this.convertBigIntToString(item)),
      );
      return result;
