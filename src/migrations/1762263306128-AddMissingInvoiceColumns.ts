...
export class AddMissingInvoiceColumns1762263306128
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "invoice" ADD "status" varchar DEFAULT 'pending'`,
    );

    // Add buyerApprovalDate column
    await queryRunner.query(
      `ALTER TABLE "invoice" ADD "buyerApprovalDate" TIMESTAMP`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "invoice" ADD "is_claimed_for_itc" boolean DEFAULT false`,
    );

    // Add foreign key constraints
    await queryRunner.query(
      `ALTER TABLE "invoice" ADD CONSTRAINT "FK_invoice_seller" FOREIGN KEY ("sellerId") REFERENCES "company"("id")`,
    );
    await queryRunner.query(
      `ALTER TABLE "invoice" ADD CONSTRAINT "FK_invoice_buyer" FOREIGN KEY ("buyerId") REFERENCES "customer"("id")`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "invoice" DROP CONSTRAINT "FK_invoice_buyer"`,
    );
    await queryRunner.query(
      `ALTER TABLE "invoice" DROP CONSTRAINT "FK_invoice_seller"`,
    );

    // Remove columns
    await queryRunner.query(
      `ALTER TABLE "invoice" DROP COLUMN "is_claimed_for_itc"`,
    );
    await queryRunner.query(`ALTER TABLE "invoice" DROP COLUMN "approvedBy"`);
    await queryRunner.query(
      `ALTER TABLE "invoice" DROP COLUMN "buyerApprovalDate"`,
    );
